  ...
  useEffect(() => {
    ...
    if (window && window.innerWidth < 768) {
      ...
      for (let i = 2; i <= totalLines; i++) {
        ...
        bgPosImg +=
          (i % 2 ? `, left` : `, right`) +
          ` ${inlinePadding} ` +
          `top calc(100% / calc(${baseBlockHeight}) * ${offset})`
        bgImgImage += ', ' + _img
      }
      // bgSize = '184px 178px';
      bgSizeImg =
        'clamp(184px, calc(184 * 100vw / 375), 324px) clamp(178px, calc(184 * 100vw / 375 / 1.057), 314px)'
    }
    ...
    if (window && window.innerWidth >= 768) {
      ...
      for (let i = 2; i <= totalLines; i++) {
        baseBgOffset +=
          ...
          (listRef.current.clientHeight -
            (window.innerWidth >= 1280 ? 140 : 100) -
            totalLines * bgSize) /
            (totalLines - 1)
        bgPosPattern += `, left top ${baseBgOffset}px`
      }
      ...
      bgImgImage =
        ', ' +
        (_img + ', ')
          .repeat(data.length > 4 ? Math.ceil(data.length / 2) * 2 : 4)
          .slice(0, -2)
