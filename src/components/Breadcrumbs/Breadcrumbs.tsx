...
export default function Breadcrumbs() {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      for (let key in params) {
        if (params[key as keyof typeof params]) {
          ...
          if (data) {
            ...
            segmentTranslation[name as string] = currentItem.title.filter(
      ...
      const breadcrumbs =
        pathArray[pathArray.length - 1] !== 'rules'
    ...
    <div className="container">
      <nav className={s.breadcrumbsWrapper}>
        {breadcrumbs.map((item, i) => (
          <BreadcrumbItem item={item} key={i} />
        ))}
      </nav>
      <button className={s.arrowBack} onClick={back}>
