...
const Hero = () => {
  ...
  useGSAP(() => {
    ...
    mm.add({
      ...
      const {isMobile, isLaptop, isDesktop} = context.conditions as gsap.Conditions;
      ...
      if(isMobile) {
        ...
        .from(
         ...
         //текст Привіт
        .from(
          [box1Ref.current, box99Ref.current],
          { x: -100}, "<"
        );
        ...
        }).from([`.${css.grandbox2} > .${css.title1}.${css.textContainer}`, box3Ref.current], {
          x: 100,
        }).from(grandpaRef.current, {
          x: -100,
        }, "<");
        ...
        }).from([photo2Animation.current, photo4Animation.current, photo3Animation.current], {
          x: i => i%2 ? -100 : 100,
          y: 50,
          opacity: 0,
          stagger: 0.3,
        ...
        gsap.timeline({
          ...
          clipPath: "inset(0% 0% 100% 0%)",
          duration: 1.5
        }).from([illustration3Ref.current, photo5Ref.current, photo6Ref.current], {
          x: i => i%2 ? 100 : -100,
          y: 50,
          opacity: 0,
          stagger: 0.3
        ...
        }).from(grandpa3Animation.current, {}).from(photo7Animation.current, {
          x: -100
        }).from(photo8Animation.current, {
          y: -50,
          x: -100
      ...
      //Затишне місце
        ...
        }).from([`.${css.grandbox2} > .${css.title1}.${css.textContainer}`, box3Ref.current], {x: -200})
        ...
        gsap.timeline({
          ...
          x: 200,
        }, ">-0.4").from(photo4Animation.current, {
          x: -200,
        }, ">-0.4")    
        //путевая линия
        .from(line2Animation.current,  {
      ...
      if(isDesktop) {
        gsap.timeline({
          ...
          }).from([box1Ref.current, photo1Animation.current, box99Ref.current],
          { x: i => i%2 ? 200 : -200 },
        ).from(grandpa1Ref.current,
          { y: -200}, "<"
        );
        ...
        }).from([box3Ref.current, grandpaRef.current, `.${css.grandbox2} > .${css.title1}`], {
          x: i => i%2 ? 200 :-200,
        }, "<");
    ...
    <section className={css.section}>
      <div className={css.box1Ref}>
        ...
        <div className={css.grandbox1} >
          <h1 className={`${css.mainTitle} ${css.textBox1}`} ref={box1Ref}>
      ...
      <div className={css.imgBox3}>
        ...
        <div className={css.line3Box}>
          {isMobile ?
            <svg viewBox="0 0 237 340" fill="none" xmlns="http://www.w3.org/2000/svg" className={`${css.svg} ${css.line3}`} preserveAspectRatio="none">
            <path d="M214.845 2.6671C215.213 3.30482 216.028 3.52332 216.666 3.15513C217.304 2.78693 217.522 1.97148 217.154 1.33376C216.786 0.696032 215.971 0.477532 215.333 0.845727C214.695 1.21392 214.477 2.02938 214.845 2.6671ZM58.8449 339.167C59.2131 339.804 60.0285 340.023 60.6662 339.655C61.304 339.287 61.5225 338.471 61.1543 337.833C60.7861 337.196 59.9706 336.977 59.3329 337.345C58.6952 337.714 58.4767 338.529 58.8449 339.167ZM215.77 2.09964C220.851 13.851 226.902 27.5755 231.007 42.1068C235.112 56.6374 237.264 71.9563 234.561 86.8921C229.16 116.729 204.347 145.164 136.553 162.681L136.678 163.165C204.562 145.625 229.602 117.094 235.053 86.9812C237.775 71.9424 235.604 56.5409 231.488 41.9709C227.373 27.4015 221.307 13.6453 216.229 1.90121L215.77 2.09964ZM136.553 162.681C102.626 171.447 74.0473 180.932 51.9673 191.703C29.8922 202.471 14.2827 214.538 6.33627 228.483C-1.62077 242.448 -1.8735 258.258 6.68087 276.445C15.2293 294.62 32.5763 315.18 59.8363 338.689L60.1628 338.311C32.9234 314.819 15.6378 294.314 7.13331 276.233C-1.36526 258.164 -1.0913 242.529 6.7707 228.731C14.6433 214.915 30.1452 202.904 52.1865 192.152C74.223 181.403 102.765 171.927 136.678 163.165L136.553 162.681Z" fill="#3F5540"/>
            </svg> :
            <svg 
              viewBox="0 0 981 440" 
